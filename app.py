...
attention_span = st.radio("학생들의 집중 유지 수준은?", [
    "단기 유지형(~7분)", "중기 유지형(8~15분)", "장기 유지형(15분 이상)"
])
...
st.markdown("---")
st.markdown("### 📋 선택 요약")
st.write(f"**교사 스타일:** {teacher_style}, {teacher_strength}, {teaching_design}")
st.write(f"**학급 특성:** {attention_span}, 산만 비율 {distraction_level}, 학습부진 비율 {low_achievement_level}, 도전행동: {challenging_behavior}, 특이요구: {special_needs}")
st.write(f"**수업 목표:** {teaching_focus}")
st.write(f"**선호 수업 형태:** {teaching_tools}")
st.write(f"**AI 피드백 요청 분야:** {coaching_focus}")
